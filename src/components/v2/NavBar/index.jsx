...
export default function NavBar() {
  ...
  return (
    <div className={classNames(styles.root)}>
      <div style={{ width: "100%" }}>
        <div
          className={classNames("d-block d-lg-none", styles.togglerBtnWrapper)}
        >
          <nav
            id="page-navbar"
          ...
          <div className={classNames("col-8")}>
            <ul className={classNames("row")}>
              {items.map(({ name, href }) => (
                <li
                  key={href}
                  className={classNames("col pt-4 pb-4", styles.navbarItem)}
                >
                  <Link
                    href={href}
                    className={classNames("p-0", styles.navItem)}
                  >
                    {name}
                  </Link>
                </li>
